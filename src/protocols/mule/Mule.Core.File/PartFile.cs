...
namespace Mule.Core.File
{
    ...
    public interface PartFile : KnownFile
    {
        ...
        void InitializeFromLink(ED2KFileLink fileLink, uint cat);
        uint Process(uint reducedownload, uint icounter);
        //filename = *.part.met
        bool LoadPartFile(string in_directory, string filename, bool getsizeonly);
        bool ImportShareazaTempfile(string in_directory, string in_filename, bool getsizeonly);
        ...
        bool GetNextRequestedBlock(UpDownClient sender, RequestedBlock newblocks, ref UInt16 count) /**/;
        void WritePartStatus(SafeMemFile file);
        void WriteCompleteSourcesCount(SafeMemFile file);
        void AddSources(SafeMemFile sources, uint serverip, UInt16 serverport, bool bWithObfuscationAndHash);
        ...
        void AddClientSources(SafeMemFile sources,
        ...
        string GetProgressString(UInt16 size);
