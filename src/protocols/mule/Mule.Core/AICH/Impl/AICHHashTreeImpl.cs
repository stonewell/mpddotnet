...
{
    class AICHHashTreeImpl : MuleBaseObjectImpl, AICHHashTree
    {
        #region Fields
        private AICHHashTree leftTree_;
        private AICHHashTree rightTree_;

        private AICHHash hash_ = null;
        ...
        public override MuleEngine MuleEngine
        {
            ...
            set
            {
                ...
                hash_ = MuleEngine.CoreObjectManager.CreateAICHHash();
            }
        }
        #endregion
        ...
        public virtual bool LoadLowestLevelHashs(FileDataIO fileInput)
        {
            ...
            else
            {
                ...
                ulong nRight = DataSize - nLeft;
                if (LeftTree == null)
                    LeftTree = new AICHHashTreeImpl(nLeft, true, (nLeft <= CoreConstants.PARTSIZE) ? CoreConstants.EMBLOCKSIZE : CoreConstants.PARTSIZE);
                if (RightTree == null)
                    RightTree = new AICHHashTreeImpl(nRight, false, (nRight <= CoreConstants.PARTSIZE) ? CoreConstants.EMBLOCKSIZE : CoreConstants.PARTSIZE);
        ...
        public bool SetHash(FileDataIO fileInput, uint wHashIdent, sbyte nLevel, bool bAllowOverwrite)
        {
            ...
            if (nLevel == 0)
            {
                ...
                if (HashValid && !bAllowOverwrite)
                {
                    ...
                    AICHHash hash = MuleEngine.CoreObjectManager.CreateAICHHash(fileInput);
        ...
        public void SetBlockHash(ulong nSize, ulong nStartPos, AICHHashAlgo pHashAlg)
        {
            byte nLevel = 0;
            ...
            //sanity
            if (pToInsert.BaseSize != CoreConstants.EMBLOCKSIZE || pToInsert.DataSize != nSize)
            {
                //TODO:Log
        ...
        public bool ReCalculateHash(AICHHashAlgo hashalg, bool bDontReplace)
        {
            if (leftTree_ != null && rightTree_ != null)
            {
        ...
        public bool VerifyHashTree(AICHHashAlgo hashalg, bool bDeleteBadTrees)
        {
            if (!HashValid)
            {
                if (bDeleteBadTrees)
            ...
            if ((rightTree_  != null && rightTree_.HashValid) && (leftTree_  != null && leftTree_.HashValid))
            {
                ...
                AICHHash CmpHash = MuleEngine.CoreObjectManager.CreateAICHHash();
                hashalg.Reset();
                hashalg.Add(leftTree_.Hash.RawHash);
                hashalg.Add(rightTree_.Hash.RawHash);
        ...
        public AICHHashTree FindHash(ulong nStartPos, ulong nSize, ref byte nLevel)
        {
            ...
            else
            {
                ...
                if (nStartPos < nLeft)
                {
                    ...
                    if (leftTree_ == null)
                        leftTree_ = MuleEngine.CoreObjectManager.CreateAICHHashTree(nLeft, true, (nLeft <= CoreConstants.PARTSIZE) ? CoreConstants.EMBLOCKSIZE : CoreConstants.PARTSIZE);
                ...
                else
                {
                    ...
                    if (rightTree_ == null)
                        rightTree_ = MuleEngine.CoreObjectManager.CreateAICHHashTree(nRight, false, (nRight <= CoreConstants.PARTSIZE) ? CoreConstants.EMBLOCKSIZE : CoreConstants.PARTSIZE);
